...
export default function ChatbotsPage() {
  ...
  const [selectedBotForQR, setSelectedBotForQR] = useState<ChatbotWithPlan | null>(null)
  ...
  const fetchQRCode = useCallback(async (chatbotId: string) => {
    if (!user) return
    ...
    try {
      ...
      try {
        ...
        if (fetchError instanceof Error) {
          if (fetchError.name === 'AbortError') {
            throw new Error('Request timeout - QR service took too long to respond')
          } else if (fetchError.message.includes('Failed to fetch')) {
            throw new Error('Unable to connect to QR service. This could be due to:\n• Network connectivity issues\n• CORS policy blocking the request\n• QR service is temporarily unavailable\n\nPlease try again later or contact support.')
          }
        }
        throw fetchError
      }
      ...
      if (!response.ok) {
        console.error(`QR API Error: ${response.status} ${response.statusText}`)
        console.error('Error response:', responseText)
        
        if (response.status === 0) {
          throw new Error('Network error: Unable to reach QR service. Please check your internet connection.')
        } else if (response.status >= 500) {
          throw new Error(`QR service is temporarily unavailable (${response.status}). Please try again later.`)
        } else if (response.status === 404) {
          throw new Error('QR service endpoint not found. Please contact support.')
        } else {
          throw new Error(`QR service error: ${response.status} ${response.statusText}`)
        }
      }
      ...
      if (data.qr) {
        ...
        console.error('No QR code in response, available keys:', Object.keys(data))
  ...
  const handleCreateBot = async () => {
    ...
    try {
      ...
      const response = await fetch('https://n8n.tanam.io/webhook/chatbot/create', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${session.access_token}`
  ...
  const getUsageText = (chatbot: ChatbotWithPlan) => {
    const percentage = chatbot.ai_quota ? Math.round(((chatbot.ai_usages || 0) / chatbot.ai_quota) * 100) : 0
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto min-w-full">
            <table className="w-full">
              <thead className="bg-gray-50 border-b">
                <tr>
                  <th className="text-left py-3 px-3 lg:px-6 text-sm font-medium text-gray-500 min-w-[150px]">Nama</th>
                  <th className="text-left py-3 px-3 lg:px-6 text-sm font-medium text-gray-500 min-w-[100px]">Status</th>
                  <th className="text-left py-3 px-3 lg:px-6 text-sm font-medium text-gray-500 min-w-[120px] hidden sm:table-cell">Paket</th>
                  <th className="text-left py-3 px-3 lg:px-6 text-sm font-medium text-gray-500 min-w-[100px] hidden md:table-cell">Perpanjangan Otomatis</th>
                  <th className="text-left py-3 px-3 lg:px-6 text-sm font-medium text-gray-500 min-w-[120px] hidden lg:table-cell">Penggunaan AI</th>
                  <th className="text-left py-3 px-3 lg:px-6 text-sm font-medium text-gray-500 min-w-[120px] hidden xl:table-cell">Berakhir Pada</th>
                  <th className="text-left py-3 px-3 lg:px-6 text-sm font-medium text-gray-500 min-w-[100px]">Aksi</th>
                </tr>
              </thead>
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={7} className="py-8 text-center text-gray-500 text-sm">
                      {searchQuery ? 'Tidak ada chatbot yang ditemukan sesuai pencarian Anda' : 'Tidak ada chatbot ditemukan'}
                    </td>
                  </tr>
                ) : (
                    ...
                    <td className="py-4 px-3 lg:px-6">
                      <div className="flex items-center space-x-3">
                        ...
                        <div className="min-w-0">
                          <div className="font-medium text-gray-900 truncate">{chatbot.name || 'Bot Tanpa Nama'}</div>
                          <div className="text-xs text-gray-500 sm:hidden">
                            {chatbot.plans?.name || 'Tanpa Paket'} • {getUsageText(chatbot)}
                          </div>
                        </div>
                      </div>
                    </td>
                    <td className="py-4 px-3 lg:px-6">
                      {getStatusBadge(chatbot.status || 'PROVISIONING', chatbot)}
                    </td>
                    <td className="py-4 px-3 lg:px-6 hidden sm:table-cell">
                      <div>
                        <div className="font-medium text-gray-900">{chatbot.plans?.name || 'Tanpa Paket'}</div>
                        <div className="text-sm text-gray-500">
                          {chatbot.plans?.price_per_month ? formatCurrency(chatbot.plans.price_per_month) : 'Gratis'}/bulan
                        </div>
                      </div>
                    </td>
                    <td className="py-4 px-3 lg:px-6 hidden md:table-cell">
                      <div className={`w-10 h-6 rounded-full ${chatbot.is_auto_renewal ? 'bg-green-500' : 'bg-gray-300'} relative cursor-pointer`}>
                        <div className={`w-4 h-4 bg-white rounded-full absolute top-1 transition-transform ${chatbot.is_auto_renewal ? 'translate-x-5' : 'translate-x-1'}`}></div>
                      </div>
                    </td>
                    <td className="py-4 px-3 lg:px-6 hidden lg:table-cell">
                      <div>
                        <div className="text-gray-900">
                          {chatbot.ai_usages?.toLocaleString() || '0'} / {chatbot.ai_quota?.toLocaleString() || '0'}
                        </div>
                        <div className="text-sm text-gray-500">
                          {chatbot.ai_quota ? Math.round(((chatbot.ai_usages || 0) / chatbot.ai_quota) * 100) : 0}% used
                        </div>
                      </div>
                    </td>
                    <td className="py-4 px-3 lg:px-6 hidden xl:table-cell">
                      <div>
                        <div className="text-gray-900">{formatDate(chatbot.expired_at)}</div>
                        <div className="text-sm text-gray-500">
          ...
          <div className="flex flex-col space-y-3 sm:flex-row sm:items-center sm:justify-between sm:space-y-0 px-3 lg:px-6 py-4 border-t">
            <div className="text-sm text-gray-500 text-center sm:text-left">
              Menampilkan {filteredChatbots.length} dari {chatbots.length} chatbot
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <RadioGroup value={selectedPlan} onValueChange={setSelectedPlan}>
                {plans.map((plan) => (
                  <div key={plan.id} className="flex items-start space-x-3 p-3 border rounded-lg hover:bg-gray-50">
              ...
              {!isBalanceSufficient() && getSelectedPlanData() && (
                <Alert className="border-red-200 bg-red-50">
                  ...
                  <AlertDescription className="text-red-700">
                    <div className="space-y-2">
                      <p>
                        Saldo tidak mencukupi. Anda membutuhkan {formatCurrency(getSelectedPlanData()!.price_per_month - balance)} lagi untuk membuat bot ini.
                      </p>
              ...
              {isBalanceSufficient() && getSelectedPlanData() && (
                <Alert className="border-green-200 bg-green-50">
                  <AlertDescription className="text-green-700">
                    ✓ Saldo mencukupi. Sisa setelah pembuatan: {formatCurrency(balance - getSelectedPlanData()!.price_per_month)}
                  </AlertDescription>
                </Alert>
              )}
            </div>
            ...
            <div className="flex flex-col space-y-3 sm:flex-row sm:space-y-0 sm:space-x-3 pt-4">
              ...
              <Button
                ...
                disabled={!botName.trim() || !selectedPlan || !isBalanceSufficient() || creating}
        ...
        <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                <DialogTitle className="truncate">{selectedBotForQR?.name || 'Bot'}</DialogTitle>
              </div>
              <div className="flex items-center space-x-2">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3 text-xs sm:text-sm text-gray-700">
              ...
              <div className="flex items-start space-x-2">
                <span className="bg-blue-600 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs font-medium mt-0.5">4</span>
                <span>Arahkan ponsel Anda ke layar ini untuk menangkap kode QR</span>
              </div>
            </div>
                ...
                <div className="w-48 h-48 sm:w-64 sm:h-64 mx-auto bg-white border border-gray-200 rounded-lg flex items-center justify-center">
                  {qrLoading ? (
                    <div className="flex flex-col items-center space-y-2">
                      ...
                      <span className="text-sm text-gray-500">Memuat QR...</span>
                    </div>
                  ) : qrCode ? (
                    <div className="p-4">
                      <QRCode 
