...
export default function BalancePage() {
  ...
  const handleTopup = async () => {
    ...
    try {
      ...
      const response = await fetch('https://n8n.tanam.io/webhook/biling/topup-balance', {
        method: 'POST',
        headers: {
      ...
      if (result.success) {
        ...
        throw new Error(result.message || result.error || 'Topup gagal diproses. Silakan periksa koneksi internet dan coba lagi.')
      }
    } catch (error) {
      console.error('Error processing topup:', error)
      const errorMessage = (error as Error).message || 'Terjadi kesalahan saat memproses topup. Silakan coba lagi.'
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50 border-b">
                  <tr>
                    <th className="text-left py-3 px-3 lg:px-6 text-sm font-medium text-gray-500">TANGGAL</th>
                    <th className="text-left py-3 px-3 lg:px-6 text-sm font-medium text-gray-500 hidden sm:table-cell">DESKRIPSI</th>
                    <th className="text-left py-3 px-3 lg:px-6 text-sm font-medium text-gray-500">TIPE</th>
                    <th className="text-right py-3 px-3 lg:px-6 text-sm font-medium text-gray-500">JUMLAH</th>
                  </tr>
                </thead>
                <tbody>
                  {paginatedTransactions.map((transaction) => (
                    <tr key={transaction.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="py-4 px-3 lg:px-6 text-right">
                        <span className={`font-medium ${transaction.type === 'usage' ? 'text-red-600' : 'text-green-600'}`}>
              ...
              {transactions.length > 0 && (
                <div className="flex items-center justify-between px-6 py-4 border-t">
                  <div className="text-sm text-gray-500 hidden sm:block">
                    Menampilkan {((transactionsPage - 1) * itemsPerPage) + 1} sampai {Math.min(transactionsPage * itemsPerPage, transactions.length)} dari {transactions.length} transaksi
                  </div>
                  <div className="flex items-center space-x-2">
                    <Button 
                      variant="outline" 
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50 border-b">
                  <tr>
                    <th className="text-left py-3 px-3 lg:px-6 text-sm font-medium text-gray-500">TANGGAL</th>
                    <th className="text-left py-3 px-3 lg:px-6 text-sm font-medium text-gray-500">JUMLAH</th>
                    <th className="text-left py-3 px-3 lg:px-6 text-sm font-medium text-gray-500">STATUS</th>
                    <th className="text-left py-3 px-3 lg:px-6 text-sm font-medium text-gray-500">AKSI</th>
                  </tr>
                </thead>
                <tbody>
              ...
              {payments.length > 0 && (
                <div className="flex items-center justify-between px-6 py-4 border-t">
                  <div className="text-sm text-gray-500 hidden sm:block">
                    Menampilkan {((paymentsPage - 1) * itemsPerPage) + 1} sampai {Math.min(paymentsPage * itemsPerPage, payments.length)} dari {payments.length} pembayaran
                  </div>
                  <div className="flex items-center space-x-2">
                    <Button 
                      variant="outline" 
