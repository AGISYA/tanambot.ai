...
export default function ChatbotManagePage() {
  ...
  const fetchChatbot = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Unable to connect to database. Please check your connection.",
      })
      router.push('/dashboard')
    } finally {
      setLoading(false)
  ...
  const handleUpdatePrompt = async () => {
    ...
    try {
      ...
      const response = await fetch('https://n8n.tanam.io/webhook/tanam.io/bot/update-prompt', {
        method: 'POST',
        headers: {
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: `Gagal mengupdate prompt: ${error instanceof Error ? error.message : 'Unknown error'}`,
      })
    } finally {
      setSaving(false)
    }
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        toast({
          variant: "success",
          title: "Success",
          description: "Chatbot berhasil diperpanjang untuk 1 bulan!",
        })
        ...
        try {
          ...
          if (errorData.message) {
            ...
            errorMessage = `Error ${errorData.code}: ${errorData.message || 'Unknown error'}`
          }
        } catch (parseError) {
  ...
  const handleSendMessage = () => {
    ...
    setTimeout(() => {
      const botResponse = {
        ...
        sender: 'bot' as const,
        message: 'Terima kasih atas pesan Anda. Ini adalah respons simulasi dari chatbot.',
        time: new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })
      }
    ...
    return (
      <div className="p-6">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">Chatbot tidak ditemukan</h1>
          <Link href="/dashboard">
                ...
                <CardContent>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                    ...
                    <div className="bg-orange-50 p-4 rounded-lg">
                      ...
                      <div className="text-sm text-gray-600">Persentase Penggunaan</div>
                    </div>
                  </div>
                  
                  <div className="space-y-3">
                    <h3 className="text-lg font-semibold">Penggunaan Token AI</h3>
                    <div className="space-y-3">
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-gray-600">Digunakan bulan ini</span>
                      ...
                      <div className="flex justify-between items-center text-sm text-gray-500">
                        ...
                        <span>{chatbot.ai_quota?.toLocaleString() || '0'} batas</span>
                      </div>
                    </div>
                  </div>
                </CardContent>
                    ...
                    <div className="border rounded-lg p-4 bg-green-50 border-green-200">
                      <div className="flex justify-between items-start mb-4">
                        <div>
                          ...
                          <p className="text-green-600">
                            {chatbot.plans?.description || 'Tidak ada deskripsi tersedia'}
                          </p>
                        </div>
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <div className="flex justify-between">
                          <span>Kuota AI</span>
                          <span className="font-medium">
                            {chatbot.plans?.ai_quota?.toLocaleString() || '0'} token
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>Perpanjangan Otomatis</span>
                          <span className={`font-medium ${
                            chatbot.is_auto_renewal ? 'text-green-600' : 'text-red-600'
                          }`}>
                    ...
                    <div className="space-y-3">
                      ...
                      <ul className="space-y-2 text-sm text-gray-600">
                        <li className="flex items-center space-x-2">
                          ...
                          <span>
                            {chatbot.plans?.ai_quota?.toLocaleString() || '0'} token AI per bulan
                          </span>
                        </li>
                        <li className="flex items-center space-x-2">
                          <div className="w-1.5 h-1.5 bg-green-500 rounded-full"></div>
                        ...
                        <div className="space-y-4">
                          <div className="text-center space-y-2">
                            <p className="text-gray-700">
                              Anda akan memperpanjang chatbot <strong>{chatbot.name}</strong> untuk 1 bulan lagi.
                            </p>
                          </div>
                          ...
                          <div className="space-y-3 p-4 bg-gray-50 rounded-lg">
                            <div className="flex justify-between items-center">
                              ...
                              <span className="text-sm">{chatbot.plans?.name}</span>
                            </div>
                            <div className="flex justify-between items-center">
                              <span className="text-sm font-medium">Biaya</span>
                              <span className="text-sm font-semibold text-green-600">
                                {chatbot.plans ? formatCurrency(chatbot.plans.price_per_month) : 'Gratis'}
                              </span>
                            </div>
                            <div className="flex justify-between items-center">
                            ...
                            <div className="flex justify-between items-center border-t pt-2">
                              <span className="text-sm font-medium">Saldo Setelah Renewal</span>
                              <span className={`text-sm font-semibold ${
                                isBalanceSufficient() ? 'text-green-600' : 'text-red-600'
                              }`}>
                                {chatbot.plans ? formatCurrency(balance - chatbot.plans.price_per_month) : formatCurrency(balance)}
                              </span>
                            </div>
                          </div>
                          ...
                          {!isBalanceSufficient() && (
                            <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
                              <p className="text-sm text-red-700">
                                ...
                                Anda membutuhkan {chatbot.plans ? formatCurrency(chatbot.plans.price_per_month - balance) : 'Rp 0'} lagi.
                              </p>
                            </div>
                          )}
